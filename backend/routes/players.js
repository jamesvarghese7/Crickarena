...
// GET /api/players/messages - Get messages for the current player
router.get('/messages', verifyFirebaseToken, async (req, res) => {
  try {
    const playerId = req.user._id;
    
    // Find messages where the player is a recipient
    const messages = await Message.find({
      'recipients.recipient': playerId,
      'recipients.recipientModel': 'Player'
    })
...
// POST /api/players/messages - Send a message from player to coach
router.post('/messages', verifyFirebaseToken, async (req, res) => {
  try {
    const { content, subject = 'Message from Player', type = 'general' } = req.body;
    const playerId = req.user._id;
    ...
    // Get player to find their coach
    ...
    if (!player.currentClub) {
      return res.status(400).json({ message: 'Player is not associated with a club' });
    }
    
    // Get club with coach populated
    const club = await Club.findById(player.currentClub._id).populate('coach');
    if (!club || !club.coach) {
      return res.status(400).json({ message: 'Club has no coach assigned' });
    }
...
// POST /api/players/messages/:messageId/reply - Reply to a message
router.post('/messages/:messageId/reply', verifyFirebaseToken, async (req, res) => {
  try {
    ...
    // Verify the player is either the sender or a recipient
    const isSender = message.sender.toString() === playerId.toString() && message.senderModel === 'Player';
    const isRecipient = message.recipients.some(recipient => 
      recipient.recipient.toString() === playerId.toString() && recipient.recipientModel === 'Player'
    );
...
// PUT /api/players/messages/:messageId/read - Mark message as read
router.put('/messages/:messageId/read', verifyFirebaseToken, async (req, res) => {
  try {
    ...
    const recipient = message.recipients.find(rec => 
      rec.recipient.toString() === playerId.toString() && rec.recipientModel === 'Player'
    );
