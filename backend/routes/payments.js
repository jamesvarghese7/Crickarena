...
router.post('/verify', verifyFirebaseToken, requireRole('clubManager'), async (req, res) => {
  try {
    ...
    // After successful payment, ensure a pending registration exists for admin approval
    try {
      const myClub = await Club.findOne({ manager: req.user._id });
      const t = await Tournament.findById(tournamentId);
      if (myClub && t) {
        const exists = (t.registrations || []).some(r => String(r.club) === String(myClub._id));
        if (!exists) {
          t.registrations.push({ club: myClub._id, status: 'pending', appliedAt: new Date() });
          await t.save();
        }
      }
    } catch (e) {
      console.warn('Post-payment registration enqueue failed (non-blocking):', e?.message || e);
    }
