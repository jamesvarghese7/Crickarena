<template>
  <div class="bg-white rounded-xl shadow-sm p-4 border border-emerald-100">
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-base font-bold text-slate-800">Training Sessions</h2>
      <button 
        @click="showCreateModal = true"
        class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-lg shadow-sm text-white bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500"
      >
    ...
    <div v-if="filteredSessions.length > 0" class="space-y-3">
      ...
      >
        <div class="flex justify-between items-start">
          <div>
            <h3 class="text-sm font-bold text-slate-800">{{ session.focusArea }}</h3>
            <p class="text-xs text-slate-500 mt-0.5">{{ session.location }}</p>
          </div>
          <span 
            :class="[
              'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium',
              getSessionStatus(session.date) === 'today' ? 'bg-amber-100 text-amber-700' :
              getSessionStatus(session.date) === 'upcoming' ? 'bg-emerald-100 text-emerald-700' :
        ...
        <div v-if="session.notes" class="mt-2 p-2 bg-emerald-50 rounded-lg">
          <p class="text-xs text-emerald-700">{{ session.notes }}</p>
        </div>
