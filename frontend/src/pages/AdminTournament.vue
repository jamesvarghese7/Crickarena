...
async function saveTournament(){
  ...
  if (form.registrationDeadline && (!isValidDateStr(form.registrationDeadline) || new Date(form.registrationDeadline) > new Date(form.startDate))) {
    errorMsg.value = 'Registration deadline must be a valid date on/before the start date';
    return;
  }

  const payload = { ...form, venues: venuesText.value.split(',').map(s=>s.trim()).filter(Boolean) }; // includes description, rules and entryFee
  try {
