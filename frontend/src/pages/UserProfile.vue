<template>
  <div class="relative min-h-screen">
    ...
    <div class="relative z-10 max-w-4xl mx-auto px-4 py-8">
      <!-- Header -->
      <div class="mb-8 glass-panel-hero rounded-3xl p-8 text-center">
        <div class="inline-flex items-center gap-2 bg-gradient-to-r from-emerald-500/20 to-green-500/20 backdrop-blur-sm border border-emerald-400/30 text-white px-4 py-2 rounded-full text-sm font-semibold mb-4">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
          </svg>
          Account Settings
        </div>
        <h1 class="text-4xl font-black text-white mb-2 neon-gradient">Profile Settings</h1>
        <p class="text-gray-300">Manage your personal information and account settings</p>
      </div>

      <!-- Profile Photo Section -->
      <section class="glass-panel rounded-3xl p-8 mb-6">
        ...
        <div class="flex flex-col md:flex-row items-center gap-8">
          ...
          <div class="relative group">
            <div class="w-32 h-32 rounded-full overflow-hidden border-4 border-green-400/50 shadow-lg transition-all duration-300 group-hover:scale-105 group-hover:border-green-400" style="box-shadow: 0 0 20px rgba(0, 255, 136, 0.3);">
              <img
                v-if="photoUrl"
                :src="photoUrl"
                alt="Profile"
            ...
            <label class="absolute bottom-0 right-0 bg-gradient-to-br from-green-500 to-emerald-600 text-white rounded-full p-3 cursor-pointer hover:from-green-600 hover:to-emerald-700 transition-all shadow-lg hover:shadow-xl" style="box-shadow: 0 0 15px rgba(0, 255, 136, 0.4);">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="flex-1 text-center md:text-left">
            <h3 class="text-lg font-semibold text-white mb-2">Update Your Photo</h3>
            <p class="text-gray-300 mb-4">Click the camera icon to upload a new profile picture</p>
            ...
            <button
              v-if="photoUrl"
              @click="removePhoto"
              class="mt-4 px-4 py-2 bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 border border-red-400/30 transition text-sm font-medium"
            >
              Remove Photo
            </button>
          </div>
      ...
      <section class="glass-panel rounded-3xl p-8 mb-6">
        ...
        <form @submit.prevent="saveProfile" class="space-y-6">
          <div class="grid md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                :value="email"
                type="email"
                class="w-full bg-white/5 border-2 border-white/20 rounded-xl px-4 py-3 text-gray-400 cursor-not-allowed"
                disabled
              />
          ...
          <div class="grid md:grid-cols-2 gap-6">
            ...
            <div>
              <label class="block text-sm font-medium text-white mb-2">District</label>
              <select
                v-model="form.district"
                class="w-full bg-white/10 backdrop-blur-sm border-2 border-green-400/30 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-green-400 transition-all duration-300"
              >
                <option value="" class="bg-gray-800">Select district</option>
                <option v-for="d in districts" :key="d" :value="d" class="bg-gray-800">{{ d }}</option>
              </select>
            </div>
          </div>
          ...
          <div class="flex justify-end">
            <button
              type="submit"
              :disabled="saving"
              class="btn-primary px-8 py-3 rounded-2xl font-bold transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
      ...
      <section class="glass-panel rounded-3xl p-8 mb-6">
        ...
        <form @submit.prevent="changePassword" class="space-y-6">
          ...
          <div class="bg-blue-500/10 border border-blue-400/30 rounded-xl p-4 backdrop-blur-sm">
            <h4 class="text-sm font-semibold text-blue-300 mb-2">Password Requirements:</h4>
          ...
          <div class="flex items-center justify-between">
            <RouterLink to="/forgot-password" class="text-sm text-green-400 hover:text-green-300 underline font-medium">
              Forgot your password?
            </RouterLink>
            <button
              type="submit"
              :disabled="changing"
              class="px-8 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-2xl font-bold hover:from-blue-600 hover:to-indigo-700 transition-all duration-300 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
      ...
      <!-- Role & Access -->
      <section class="glass-panel-strong rounded-3xl p-8">
        <h2 class="text-2xl font-semibold text-white mb-6 flex items-center gap-2">
          <svg class="w-6 h-6 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="premium-card rounded-2xl p-6">
          <div class="flex items-center gap-4">
            <div class="w-14 h-14 bg-gradient-to-br from-green-500 to-emerald-600 rounded-full flex items-center justify-center" style="box-shadow: 0 0 20px rgba(0, 255, 136, 0.4);">
              <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
            </div>
            <div class="flex-1">
              <p class="text-sm text-gray-400 mb-1">Your Current Role</p>
              <p class="text-2xl font-bold text-white capitalize">{{ currentRole }}</p>
            </div>
            <div class="px-4 py-2 bg-green-500/20 rounded-lg border border-green-400/40 text-green-400 font-medium text-sm">
              Active
            </div>
          </div>
        </div>
