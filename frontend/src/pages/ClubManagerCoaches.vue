                    ...
                    <div v-if="application.coach?.coachingHistory && application.coach.coachingHistory.length > 0" class="mt-3 p-3 bg-slate-50 rounded-xl">
                      ...
                      <div class="space-y-3">
                        <div v-for="(history, index) in application.coach.coachingHistory" :key="index" 
                          ...
                          <div v-if="history.achievements" class="text-xs text-slate-600 mt-1">
                            <span class="font-medium">Achievements:</span> {{ history.achievements }}
                          </div>
                          <div v-if="history.description" class="text-xs text-slate-600 mt-1">
              ...
              <div class="text-center">
                ...
                <div class="grid grid-cols-3 gap-2 mb-4">
                  <div class="text-center p-2 bg-blue-50 rounded-lg">
                    <div class="text-lg font-bold text-blue-600">{{ coach.playersCoached }}</div>
                    <div class="text-xs text-blue-500">Players</div>
                  </div>
                  <div class="text-center p-2 bg-green-50 rounded-lg">
                    <div class="text-lg font-bold text-green-600">{{ coach.yearsOfExperience }}</div>
                    <div class="text-xs text-green-500">Years</div>
                  </div>
                  <div class="text-center p-2 bg-orange-50 rounded-lg">
                    <div class="text-lg font-bold text-orange-600">{{ coach.tournamentsWon }}</div>
                    <div class="text-xs text-orange-500">Tournaments</div>
                  </div>
                </div>
                ...
                <div class="text-xs text-slate-500 mb-4 space-y-1">
                  ...
                  <div v-if="coach.phone" class="flex items-center justify-center gap-1">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto my-8">
        ...
        <div v-if="selectedCoach" class="p-6">
          ...
          <div v-if="selectedCoach.coachingHistory && selectedCoach.coachingHistory.length > 0" class="mt-6">
            ...
            <div v-for="(history, index) in selectedCoach.coachingHistory" :key="index" class="mt-3 p-3 bg-gray-50 rounded-lg">
              <div v-if="history.achievements" class="text-sm">
                <span class="font-medium text-gray-700">Achievements for {{ history.teamName }}:</span>
                <span class="text-gray-600 ml-1">{{ history.achievements }}</span>
              </div>
            </div>
          </div>
              ...
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="(cert, index) in selectedCoach.certifications" :key="index">
                    <td class="px-4 py-2 text-sm text-gray-900">
                      <div>{{ cert.name }}</div>
                      <div v-if="cert.certificateNumber" class="text-xs text-gray-500">ID: {{ cert.certificateNumber }}</div>
                    </td>
                    <td class="px-4 py-2 text-sm text-gray-900">{{ cert.issuingBody }}</td>
                    <td class="px-4 py-2 text-sm text-gray-900">{{ cert.level }}</td>
                    <td class="px-4 py-2 text-sm text-gray-900">
                      <div>{{ cert.yearObtained }}</div>
                      <div v-if="cert.expiryDate" class="text-xs text-gray-500">Expires: {{ formatDate(cert.expiryDate) }}</div>
                    </td>
                  </tr>
                </tbody>
              </table>
...
const fetchCoachApplications = async () => {
  try {
    ...
    approvedCoaches.value = allApplications.filter(app => app.status === 'approved')
    
    // Transform approved applications into coach objects with proper structure
    approvedCoaches.value = approvedCoaches.value.map(app => ({
      _id: app.coach._id,
