...
async function register(t){
  ...
  try {
    // If entry fee is required, initiate payment first and only then create/approve registration via backend
    if (Number(t.entryFee) > 0) {
      if (!confirm(`This tournament requires an entry fee of ₹${Number(t.entryFee).toFixed(0)}. Continue to payment?`)) {
        return;
      }
      const { data } = await api.post('/payments/orders', { tournamentId: t._id });
      ...
      // Free tournaments: proceed with normal registration
